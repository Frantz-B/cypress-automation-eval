  ...
  describe('Bidders API', () => {
    ...
    it('Retrieve the bidder', () => {
      ...
      cy.request(requestOptions)
        .then((resp) => {
          ...
          assert.equal(resp.body.user_sync_url_secure, secureUrl, ['secure url ']);
          assert.equal(resp.body.user_sync_url, unSecureurl, ['un-secure url ']);
          assert.equal(resp.body.send_not_synced_users, notSyncedUsers, ['send not synced users ']);
          assert.equal(resp.body.iframe_wrapper, iFrame, ['iFrame wrapper']);
          assert.equal(resp.body.send_video_traffic, vidTraffic, ['video traffic']);
          assert.equal(resp.body.send_native_traffic, nativeTraffic, ['Native traffic ']);
          assert.equal(resp.body.gzip_compression, gzipComp, ['gzip compression ']);
        });
    });
    ...
    it('Update the bidder', () => {
      cy.fixture('bidder.json').then((bidderJson) => {
        ...
        reqBody.send_not_synced_users = 1;
        reqBody.send_native_traffic = 1;
        reqBody.gzip_compression = 1;
        reqBody.direct_like_cookie_sync = 1;
        reqBody.iframe_wrapper = 1;
        const requestOptions = getRequest({
          method: 'PATCH',
          body: reqBody,
        });
    ...
    it('Pushing the created Bidder to athena', () => {
      ...
      cy.request(requestOptions).then((resp) => {
        athenaId = resp.body.data.bidderID;
        assert.equal(resp.status, 200, ['response status value ']);
      });
    });
