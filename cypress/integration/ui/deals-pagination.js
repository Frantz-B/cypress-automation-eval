...
context('Deals Pagination', () => {
  describe('Deals Pagination UI', () => {
    beforeEach(() => {
      cy.setCookie('kauth_access', Cypress.env('authToken'));
    });

    it('Navigate to Deals page', () => {
      cy.visit('');
      cy.get('[data-qa="deals-dashboards--select-deals"]').click({ force: true }).wait(2500).then(() => {
